...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="挂号费" prop="guahaofei">
            <el-input v-model.number="ruleForm.guahaofei" 
                placeholder="挂号费" clearable :disabled=" false  ||ro.guahaofei"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="接诊时间" prop="jiezhenshijian">
            <el-input v-model="ruleForm.jiezhenshijian" 
                placeholder="接诊时间" clearable :disabled=" false  ||ro.jiezhenshijian"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="接诊范围" prop="jiezhenfanwei">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="接诊范围"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="医生简介" prop="yishengjianjie">
            <editor 
...
<script>
  export default {
      ...
      yishenggonghaoChange () {
        this.$http.get('follow/yisheng/yishenggonghao?columnValue=' + this.ruleForm.yishenggonghao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if(res.data.data.keshi){
              this.ruleForm.keshi = res.data.data.keshi
            }
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chongwuyiyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chongwuyiyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chongwuyiyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
