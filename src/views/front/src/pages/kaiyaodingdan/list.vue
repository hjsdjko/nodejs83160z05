...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 20px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
	  ...
	  <el-button v-if="btnAuth('kaiyaodingdan','新增')" :style='{"cursor":"pointer","border":"1px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","outline":"none","color":"#333","borderRadius":"0px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/kaiyaodingdanAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"55%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">{{item.yaopinmingcheng}}</div>
		        ...
		        <div :style='{"padding":"2px 10px 0 0","borderColor":"#f0f0f0","borderStyle":"dashed","borderWidth":"0 0 0px 0","display":"inline-block"}' class="publisher">
		          <span class="icon iconfont icon-geren16"></span>
		          <span class="text">{{item.yishenggonghao}}</span>
		        </div>
		      </div>
		      <div class="desc" v-html="item.yongyaoshuoming"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/yaopinxinxi/yaopinmingcheng').then(res => {
			if (res.data.code == 0) {
				this.yaopinmingchengOptions = res.data.data;
				this.selectOptionsList.push({name:'药品名称',list:this.yaopinmingchengOptions,tableName: 'yaopinmingcheng',check: -1})
			}
		});
		this.ispayOptions = "已支付,未支付".split(',')
		this.selectOptionsList.push({name:'是否支付',list:this.ispayOptions,tableName: 'ispay',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`kaiyaodingdan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/kaiyaodingdanDetail', query: params});
      },
