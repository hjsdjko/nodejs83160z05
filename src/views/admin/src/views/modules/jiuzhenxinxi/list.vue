<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">宠物名称</label>
						<el-input v-model="searchForm.chongwumingcheng" placeholder="宠物名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jiuzhenxinxi','医嘱')" @click="yizhuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								医嘱
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiuzhenxinxi','开药')" @click="kaiyaodingdanCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								开药
							</el-button>
		...
		<yizhuxinxi-cross-add-or-update v-if="yizhuxinxiCrossAddOrUpdateFlag" :parent="this" ref="yizhuxinxiCrossaddOrUpdate"></yizhuxinxi-cross-add-or-update>
		<kaiyaodingdan-cross-add-or-update v-if="kaiyaodingdanCrossAddOrUpdateFlag" :parent="this" ref="kaiyaodingdanCrossaddOrUpdate"></kaiyaodingdan-cross-add-or-update>
...
import yizhuxinxiCrossAddOrUpdate from "../yizhuxinxi/add-or-update";
import kaiyaodingdanCrossAddOrUpdate from "../kaiyaodingdan/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				yizhuxinxiCrossAddOrUpdateFlag: false,
				kaiyaodingdanCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			yizhuxinxiCrossAddOrUpdate,
			kaiyaodingdanCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			yizhuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yizhuxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yizhuxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yizhuxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			kaiyaodingdanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kaiyaodingdanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiuzhenxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kaiyaodingdanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kaiyaodingdanCrossaddOrUpdate.init(row.id,type);
				});
			},
