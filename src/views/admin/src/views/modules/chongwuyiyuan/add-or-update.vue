	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="接诊时间" prop="jiezhenshijian">
					<el-input v-model="ruleForm.jiezhenshijian" placeholder="接诊时间" clearable  :readonly="ro.jiezhenshijian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="接诊时间" prop="jiezhenshijian">
					<el-input v-model="ruleForm.jiezhenshijian" placeholder="接诊时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="textarea" v-if="type!='info'" label="接诊范围" prop="jiezhenfanwei">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="接诊范围"
					  v-model="ruleForm.jiezhenfanwei" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.jiezhenfanwei" label="接诊范围" prop="jiezhenfanwei">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}'>{{ruleForm.jiezhenfanwei}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jiezhenfanwei'){
							this.ruleForm.jiezhenfanwei = obj[o];
							this.ro.jiezhenfanwei = true;
							continue;
						}
						if(o=='jiezhenshijian'){
							this.ruleForm.jiezhenshijian = obj[o];
							this.ro.jiezhenshijian = true;
							continue;
						}
						if(o=='yishengjianjie'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.yishengjianjie = this.ruleForm.yishengjianjie.replace(reg,'../../../nodejs83160z05/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `chongwuyiyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chongwuyiyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
