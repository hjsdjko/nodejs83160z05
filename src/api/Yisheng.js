...
import YishengModel from '../models/YishengModel'
...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await YishengModel.findOne({ where: { yishenggonghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.yishenggonghao,
					role: '管理员',
                    loginUserColumn: 'yishenggonghao',
					yishenggonghao:userinfo.dataValues.yishenggonghao,
					yishengxingming:userinfo.dataValues.yishengxingming,
					...
					keshi:userinfo.dataValues.keshi,
                    tableName: 'yisheng'
			...
			userinfo.dataValues.loginUserColumn = 'yishenggonghao'
			userinfo.dataValues.tableName = 'yisheng'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await YishengModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YishengModel.findOne({ where: { id:jwt.decode(req.headers.token).id} }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
    		...
    		let pwd = '123456'
			await YishengModel.update({
				mima: pwd
			}, {
				where: {
				  yishenggonghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let result = await YishengModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await YishengModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await YishengModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			let yishenggonghaocount = await YishengModel.count({
				where: {
					yishenggonghao: req.body["yishenggonghao"]
				}
			})
            if (yishenggonghaocount > 0) {
				toRes.session(res, -1, '医生工号已存在')
            }
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await YishengModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YishengModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yisheng WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "yisheng"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM yisheng " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM yisheng " + where + " GROUP BY " + xColumnName + " DESC"
