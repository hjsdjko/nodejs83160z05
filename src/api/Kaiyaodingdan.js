...
import KaiyaodingdanModel from '../models/KaiyaodingdanModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let yaopinmingcheng = req.query.yaopinmingcheng
			if (yaopinmingcheng) {

				if (yaopinmingcheng.indexOf('%') != -1) {
					where.yaopinmingcheng = {
						[Op.like]: yaopinmingcheng
					}
				} else {
					where.yaopinmingcheng = {
						[Op.eq]: yaopinmingcheng
					}
				}
			}
			let yaopinfenlei = req.query.yaopinfenlei
			if (yaopinfenlei) {

				if (yaopinfenlei.indexOf('%') != -1) {
					where.yaopinfenlei = {
						[Op.like]: yaopinfenlei
					}
				} else {
					where.yaopinfenlei = {
						[Op.eq]: yaopinfenlei
			...
			let yongyaoshuoming = req.query.yongyaoshuoming
			if (yongyaoshuoming) {

				if (yongyaoshuoming.indexOf('%') != -1) {
					where.yongyaoshuoming = {
						[Op.like]: yongyaoshuoming
					}
				} else {
					where.yongyaoshuoming = {
						[Op.eq]: yongyaoshuoming
					}
				}
			}
			let kaiyaoshijian = req.query.kaiyaoshijian
			if (kaiyaoshijian) {

				if (kaiyaoshijian.indexOf('%') != -1) {
					where.kaiyaoshijian = {
						[Op.like]: kaiyaoshijian
					}
				} else {
					where.kaiyaoshijian = {
						[Op.eq]: kaiyaoshijian
			...
			let chongwumingcheng = req.query.chongwumingcheng
			if (chongwumingcheng) {

				if (chongwumingcheng.indexOf('%') != -1) {
					where.chongwumingcheng = {
						[Op.like]: chongwumingcheng
					}
				} else {
					where.chongwumingcheng = {
						[Op.eq]: chongwumingcheng
					}
				}
			}
			...
			if(tableName == 'yisheng') {
				where.yishenggonghao = {
					[Op.eq]: req.session.userinfo === undefined ? jwt.decode(req.headers.token).yishenggonghao : req.session.userinfo.yishenggonghao
				}
			...
			let result = await KaiyaodingdanModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await KaiyaodingdanModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await KaiyaodingdanModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await KaiyaodingdanModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await KaiyaodingdanModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await KaiyaodingdanModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await KaiyaodingdanModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
        if(tableName == 'yisheng') {
            where += " AND yishenggonghao = '" + jwt.decode(req.headers.token).username + "' ";
        }
        if(tableName == 'yonghu') {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM kaiyaodingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM kaiyaodingdan " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("kaiyaodingdan" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM kaiyaodingdan " + where + " GROUP BY " + xColumnName + " DESC"
