...
import ChongwuyiyuanModel from '../models/ChongwuyiyuanModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let jiezhenfanwei = req.query.jiezhenfanwei
			if (jiezhenfanwei) {

				if (jiezhenfanwei.indexOf('%') != -1) {
					where.jiezhenfanwei = {
						[Op.like]: jiezhenfanwei
					}
				} else {
					where.jiezhenfanwei = {
						[Op.eq]: jiezhenfanwei
			...
			let jiezhenshijian = req.query.jiezhenshijian
			if (jiezhenshijian) {

				if (jiezhenshijian.indexOf('%') != -1) {
					where.jiezhenshijian = {
						[Op.like]: jiezhenshijian
					}
				} else {
					where.jiezhenshijian = {
						[Op.eq]: jiezhenshijian
					}
				}
			}
			let yishengjianjie = req.query.yishengjianjie
			if (yishengjianjie) {

				if (yishengjianjie.indexOf('%') != -1) {
					where.yishengjianjie = {
						[Op.like]: yishengjianjie
					}
				} else {
					where.yishengjianjie = {
						[Op.eq]: yishengjianjie
					}
				}
			}
			...
			let result = await ChongwuyiyuanModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await ChongwuyiyuanModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await ChongwuyiyuanModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await ChongwuyiyuanModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ChongwuyiyuanModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await ChongwuyiyuanModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ChongwuyiyuanModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chongwuyiyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "chongwuyiyuan"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM chongwuyiyuan " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("chongwuyiyuan" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM chongwuyiyuan " + where + " GROUP BY " + xColumnName + " DESC"
